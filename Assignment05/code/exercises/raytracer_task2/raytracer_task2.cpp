...
std::shared_ptr<rt::Scene> makeMeshScene(std::string fileName,bool addArrows)
{
  ...
  if(mesh->loadFromOBJ(gDataPath+fileName))
  {
    ...
    std::shared_ptr<rt::Material> materialObject = std::make_shared<rt::PhongMaterial>(rt::Vec3d(0.9,0.9,0.0),0.5,  50.0);
      
    mesh->setMaterial(materialObject);
    mesh->transform().scale(0.06,0.06,0.06).rotate(90, 1, 0, 0).rotate(90,0,0,1).translate(0.5,0.5,0);
    // mesh->transform().rotate(150,0,0,1).scale(0.1,0.1,0.1);
...
int main(int argc, char** argv)
{
  ...
  //uncomment the line below for the bonus task
    ...
    // CHANGED THIS FOR BONUS TASK!
    std::shared_ptr<rt::Scene> scene = makeMeshScene("assets/FinalBaseMesh.obj",showArrows);
